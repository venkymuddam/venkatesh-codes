...
private:
    deque<Doctor> doctors; // Using deque instead of vector
...
void DoctorManager::create() {
    ...
    doctors.push_back(newDoctor); // Add to deque
...
int DoctorManager::findIndexById(string DoctorID) {
    auto it = find_if(doctors.begin(), doctors.end(), 
                      [&DoctorID](const Doctor& doc) { return doc.DoctorID == DoctorID; });

    if (it != doctors.end()) {
        return distance(doctors.begin(), it); // Calculate the index
    }
...
void DoctorManager::deleteById() {
    ...
    doctors.erase(doctors.begin() + index); // Remove from deque
    cout << "Doctor deleted successfully.\n";
}

DoctorManager::DoctorManager() {
    // No initialization required for deque
}
